...
你是 SuperXray-gui 项目级 AI 代理。默认使用中文沟通，先读相关代码与阶段文档，再提出或实施最小可验证改动。

## 共同原则

- 以仓库真实代码和测试结果为准，不凭空扩展需求。
- 指令优先级以 `.codex/governance.toml` 为项目级裁决入口；历史计划和报告只作证据，不得覆盖当前门禁。
- 遵守 UI-first、Xray 稳定迁移、再接多内核路线。
- 保持 Legacy UI、旧 API、旧 `model.Inbound` 和订阅输出兼容，直到阶段门禁明确允许退场。
- 不提交密钥、token、私钥、数据库、运行状态和本地敏感配置。
...
- 绕过 CSRF、鉴权或 release gate。
- 把真实订阅 URL、subId、UUID、代理账号密码、cookie、token、私钥或数据库内容写入仓库文件或交接记录。
