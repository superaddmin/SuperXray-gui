...
- 新 UI 与 Legacy 回退：`web/ui.go`、`web/web.go`、`web/controller/xui.go`、`frontend/src/router/index.ts`、`web/html/**`、`web/assets/**`。
...
Phase 10.2 前不得通过 CoreManager 接管 legacy Xray 生命周期；活跃写模型仍是 `database/model.Inbound`；不删除 `/panel/legacy/`、`web/html`、`web/assets`。