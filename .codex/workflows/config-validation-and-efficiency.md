...
- `.codex/governance.toml` 是否为 `version = 3`，是否包含 `[codex_validation]` 与五张 context map 的 first-read。
- `.codex/routing.toml` 是否存在未知 agent 引用，并将验证脚本纳入 `codex-governance` / `project-skills` 路由。
- 技能 `SKILL.md` frontmatter 与 `agents/openai.yaml` 基本字段是否存在。
- 新增运行态知识文件与网络调试 checklist 是否存在并被索引。
...
- 技能或 agent 改动：运行验证器单测、配置验证和 `quick_validate.py`。
