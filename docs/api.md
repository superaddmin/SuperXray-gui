...
所有 `/panel/api/` 下的 API 端点均启用 CSRF 中间件保护。非安全 HTTP 方法（POST/PUT/DELETE/PATCH）的请求必须携带 `X-Requested-With: XMLHttpRequest` 请求头，否则返回 `403 Forbidden`。
...
|-------------|------|
| `200` | 请求成功 |
| `403` | CSRF 验证失败（缺少 `X-Requested-With` 头） |
...
以下路由返回 HTML 页面（需要认证）。当前旧版 HTML 面板位于 `/panel/legacy/*`，`/panel/legacy` 会重定向到 `/panel/legacy/`：

| 方法 | 路径 | 功能 |
|------|------|------|
| `GET` | `/panel/legacy` | 重定向到旧版面板首页 |
| `GET` | `/panel/legacy/` | 旧版面板首页（状态仪表盘） |
| `GET` | `/panel/legacy/inbounds` | 旧版 Inbounds 管理页 |
| `GET` | `/panel/legacy/settings` | 旧版面板设置页 |
| `GET` | `/panel/legacy/xray` | 旧版 Xray 配置页 |
...
Core API 位于 `/panel/api/cores`，用于查看和操作面板注册的代理核心实例。当前源码注册两个实例：

| 实例 ID | 类型 | 能力说明 |
|---------|------|----------|
| `default-xray` | `xray` | 只读查看既有 Xray 状态；生命周期操作返回不支持 |
| `experimental-sing-box` | `sing-box` | 外部二进制实验适配器；支持校验、启动、停止、重启 |
...
| `source` | string | 实例来源 |
| `lifecycleOwner` | string | 生命周期管理方 |
| `status` | object | 状态、版本、PID、错误信息等 |
| `capabilities` | object | read/write/validate/start/stop/restart 能力位 |
| `writeSupported` | bool | 是否支持写配置 |
| `managerAttached` | bool | 是否由 CoreManager 管理生命周期 |
| `experimentalOnly` | bool | 是否为实验性实例 |
...
校验指定核心实例配置。`experimental-sing-box` 会执行 `sing-box check -c <config>`；`default-xray` 当前返回生命周期不支持。

### POST /panel/api/cores/instances/:id/start

启动指定核心实例。当前仅 `experimental-sing-box` 支持。

### POST /panel/api/cores/instances/:id/stop

停止指定核心实例。当前仅 `experimental-sing-box` 支持。

### POST /panel/api/cores/instances/:id/restart

重启指定核心实例。当前仅 `experimental-sing-box` 支持。
...
获取 Base64 编码的订阅链接；浏览器直接打开该地址时会渲染旧版兼容的可视化订阅信息页，包含 URI / JSON / Clash 订阅二维码、客户端导入菜单、流量与过期信息。
...
- 普通客户端请求：按 `subEncrypt` 返回 Base64 编码或明文协议链接列表
- 浏览器请求（`Accept: text/html`）、`?html=1` 或 `?view=html`：返回可视化订阅页面
