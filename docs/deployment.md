...
| 订阅服务 | 默认 `2096` | URI/Base64、JSON、Clash/Mihomo 订阅入口 |
...
| 订阅服务 | `2096/tcp` | 按需 | 直接公网访问订阅链接时开放；推荐通过反代统一到 `443` |
...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/superaddmin/SuperXray-gui/main/install.sh) v3.0.22
```
...
```text
ghcr.io/superaddmin/superxray-gui:3.0.22
...
services:
  3xui:
    image: ghcr.io/superaddmin/superxray-gui:3.0.22
...
当前订阅服务由独立的 Sub Server 提供，默认监听 `2096/tcp`，与 Web 面板端口相互独立。启用订阅后，每个客户端的 `subId` 可拼接到以下路径：

| 输出格式 | 默认路径 | 典型用途 |
|----------|----------|----------|
| URI / Base64 | `/sub/<subId>` | V2rayN、Shadowrocket、通用分享链接客户端 |
| Xray JSON | `/json/<subId>` | Xray 原生客户端或需要完整 JSON 的 headless 场景 |
| Clash/Mihomo | `/clash/<subId>` | Clash Meta、Mihomo、Stash 等客户端 |
...
- 使用 Nginx/Caddy 反向代理时，推荐只公网开放 `443/tcp`，把 `/sub/`、`/json/`、`/clash/` 转发到 `127.0.0.1:2096`。
- 订阅链接、`subId`、客户端 UUID、Reality 私钥和节点凭据都属于敏感信息，不应写入 Issue、截图、日志或公开文档。

### 9.2 VLESS + TCP + Reality 基线配置

VLESS Reality 入站保存前必须提供有效 `target`，否则 Xray 会拒绝启动并在日志中输出 `please fill in a valid value for "target"`。推荐基线：
...
| 安全层 | `reality` |
| `decryption` | `none` |
| Reality `target` | `<真实站点域名>:443` |
| Reality `serverNames` | 至少包含与 `target` 匹配的真实 SNI |
| `shortIds` | 至少一个十六进制短 ID |
| 入站 `tag` | 稳定命名，例如 `inbound-443` |

如需降低客户端配置试错，先在测试入站上完成“保存 → 重启 Xray → 导出订阅 → 客户端导入 → 访问测试”，再切换生产流量。Reality 的 `target` 和 `serverNames` 应选择长期稳定、TLS 行为正常的目标站点；不要留空，也不要写入不可解析或与 SNI 不匹配的值。

### 9.3 AI 平台分流与住宅出口

对于 OpenAI/ChatGPT、Anthropic/Claude、Google/Gemini 等对出口质量敏感的平台，推荐使用 Xray 的 `outbounds`、`routing.rules`、`routing.balancers`、`observatory` 与 DNS 配置做域名级分流：AI 相关域名走专用住宅 SOCKS 出口，其他流量保持 `direct` 或常规线路。完整配置模板、健康检测、DNS 策略、泄漏防护边界和回滚步骤见 [AI 平台智能分流与住宅出口运行手册](ai-routing-and-residential-egress.md)。
...
- 如果仓库只有预发布版本，使用显式版本安装命令，例如：`bash <(curl -Ls https://raw.githubusercontent.com/superaddmin/SuperXray-gui/main/install.sh) v3.0.22`。
- 换用可访问 GitHub 的网络环境。
