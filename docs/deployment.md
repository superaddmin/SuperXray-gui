...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/superaddmin/SuperXray-gui/main/install.sh) v3.0.21
```
...
```text
ghcr.io/superaddmin/superxray-gui:3.0.21
...
services:
  3xui:
    image: ghcr.io/superaddmin/superxray-gui:3.0.21
...
- 如果仓库只有预发布版本，使用显式版本安装命令，例如：`bash <(curl -Ls https://raw.githubusercontent.com/superaddmin/SuperXray-gui/main/install.sh) v3.0.21`。
- 换用可访问 GitHub 的网络环境。
