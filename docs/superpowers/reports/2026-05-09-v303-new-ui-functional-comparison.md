...
> **状态更新（2026-05-15）**：本报告记录的是 `v3.0.3` 对比时点的静态审计结论。后续版本已经补齐 Inbounds 顶部“导出全部分享链接 / 导出全部订阅”、单入站“导出链接 / 导出订阅 / JSON / QR / Clone / Reset”等入口，并在 `v3.0.11` 修复订阅公开 URI 为空时新 UI 生成 0 条订阅链接的问题。阅读本文中“未提供完整入口”“二维码/导出订阅未迁移”等结论时，应以此状态更新和当前源码为准。
